...
const navItems: Array<{
  ...
  { value: "agents", label: "AI Ops", caption: "Manager + worker", icon: BrainCircuit },
  ...
  return (
    ...
    detail={"Compare market odds with the " + model.grid + " Dixon–Coles fair-price surface."}
      ...
      <section className="panel overflow-hidden rounded-2xl">
        ...
        <div className="border-t border-slate-800/80 px-4 py-3 sm:px-5">
          ...
          <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
            {visibleMarkets.flatMap((market) => (market.goalMarkets ?? []).filter((item) => item.marketType !== "TEAM_TOTALS").map((item) => (
              <div key={`${market.fixtureId}-${item.marketType}-${item.selection}-${item.line ?? 0}`} className="rounded-lg border border-slate-800 bg-slate-950/25 px-3 py-2 text-xs">
                ...
                {(() => { const goalKey = `${market.fixtureId}-${item.marketType}-${item.selection}-${item.line ?? 0}`; const goalStake = goalStakeOverrides[goalKey] ?? (item.marketOdds ? "10.00" : "0.00"); return <div className="mt-2 flex items-center gap-2"><label className="flex min-w-0 flex-1 items-center gap-1 text-[11px] text-slate-500">€<input type="number" min="0.01" step="0.01" inputMode="decimal" value={goalStake} disabled={!item.marketOdds || item.action !== "BET"} onChange={(event) => setGoalStakeOverrides((current) => ({ ...current, [goalKey]: event.target.value }))} aria-label={`Stake for ${market.homeTeam} versus ${market.awayTeam} ${item.selection} ${item.line ?? ""}`} className="min-w-0 w-full rounded-md border border-slate-700 bg-slate-950/70 px-2 py-1 font-mono text-xs text-slate-200 outline-none focus:border-indigo-400" /></label><Button size="sm" variant="outline" disabled={!item.marketOdds || item.action !== "BET" || busy} onClick={() => onPlace(market.fixtureId, item.marketType === "BTTS" ? `BTTS_${item.selection}` : `${item.selection}_${item.line}`, Number(goalStake))} className="h-7 shrink-0 border-slate-700 bg-slate-900/50 text-xs text-slate-300 hover:bg-slate-800"><Play size={12} className="mr-1" /> {item.marketOdds ? "Paper bet" : "Fair only"}</Button></div>; })()}
              </div>
            ))) }
          </div>
        </div>
  ...
  const renderAgent = (run: AgentDashboard["latest"][number] | undefined, fallback: string) => (
    <section className="panel rounded-2xl p-5">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">{run?.task ?? "Awaiting the next Windows worker cycle."}</p>
          </div>
        </div>
      ...
      <p className="mt-5 min-h-[48px] text-sm leading-6 text-slate-300">
        {run?.summary ?? "No report has been synced from the local Windows worker yet."}
      </p>
  ...
  return (
    <div className="space-y-6">
      <SectionHeading
        ...
        title="Manager + Worker control room"
        detail="Luna Max reviews deterministic facts; Python owns the model, risk gates, and paper ledger."
        action={
          <Button
            size="sm"
            onClick={onBacktest}
...
function SettingsView({ overview, research }: { overview: Overview; research: Research }) {
  ...
  return (
    <div className="space-y-6">
      <SectionHeading eyebrow="Workspace configuration" title="Settings" detail="Sources, diagnostics, research services and worker readiness in one place." />
      ...
      <section className="panel rounded-2xl p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-slate-500">Hard limits applied by both the hosted paper ledger and the Windows worker.</p>
          </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.1fr_1fr]">
        ...
        <section className="panel rounded-2xl p-5">
          <div className="flex items-center justify-between"><div><p className="text-sm font-medium text-slate-200">Worker health</p><p className="mt-1 text-xs text-slate-500">Background cycles and heartbeat status.</p></div><Activity size={17} className="text-indigo-300" /></div>
          {research.heartbeats.length === 0 ? <p className="mt-5 text-sm text-slate-500">No heartbeat recorded yet.</p> : research.heartbeats.map((heartbeat) => <div key={heartbeat.workerName} className="mt-4 flex items-center justify-between gap-3 border-b border-slate-800 pb-3 text-sm"><div><p className="text-slate-300">{heartbeat.workerName}</p><p className="mt-1 text-xs text-slate-500">{heartbeat.cyclesCompleted} cycles · {formatDate(heartbeat.lastFinishedAt)}</p></div><StatusPill tone={heartbeat.status === "READY" ? "emerald" : "amber"}>{heartbeat.status}</StatusPill></div>)}
        </section>
      </div>
...
function LunaAssistant({ activeSection, onPaperBet }: { activeSection: View; onPaperBet: () => Promise<void> }) {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      role: "assistant",
      content: "I’m Luna, your QVM assistant. Ask me about the model, edge calculations, risk controls, paper positions, or how to use this workbench. After reviewing a market, type “place paper bet on fixture 478 away €50” to create a paper bet after the risk checks pass.",
    },
  ]);
      ...
      {open && (
        ...
        >
          <div className="flex items-center justify-between border-b border-slate-800 px-4 py-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500">QVM expert · paper mode only</p>
              </div>
            </div>
          ...
          <div className="border-t border-slate-800 px-3 py-3">
            ...
            <p className="mt-2 text-[10px] leading-4 text-slate-600">Luna explains the paper-trading system. It does not place bets or provide guaranteed outcomes.</p>
          </div>
        </section>
      )}
...
export default function Home() {
  ...
  const loadAll = useCallback(async () => {
    ...
    async function fetchJson(path: string) {
      const response = await fetch(path, { headers: { Accept: "application/json" }, signal: AbortSignal.timeout(45000) });
      const payload = await response.json().catch(() => ({}));
  ...
  return (
    ...
    >
      ...
      <main className="min-w-0 flex-1">
        <header className="sticky top-0 z-10 border-b border-slate-800/80 bg-[#07111f]/85 px-4 py-4 backdrop-blur-xl sm:px-6 lg:px-8">
            ...
            {loading ? (
              ...
              <>
                ...
                <TabsContent value="edge" className="mt-0">
                  {markets && overview && <EdgeFinderView markets={markets} model={overview.model} activeLeague={activeLeague} onScan={() => runAction("/api/qvm/scan")} onPlace={(fixtureId, selection, stakeAmount) => runAction("/api/qvm/bets", { fixtureId, selection, stakeAmount }, "Paper position created.")} busy={busy} />}
                </TabsContent>
                ...
                <TabsContent value="agents" className="mt-0">
                  {agents && <AgentOpsView agents={agents} onBacktest={() => runAction("/api/qvm/backtest", undefined, "Walk-forward evaluation completed.")} busy={busy} />}
                </TabsContent>
