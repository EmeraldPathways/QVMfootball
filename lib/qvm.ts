...
export function normaliseName(name: string) {
  const compact = compactTeamName(name);
  return TEAM_ALIASES[compact] ?? compact;
}

export function buildTeamNameMap<T extends { name: string }>(rows: T[]) {
  ...
  for (const row of rows) {
    ...
    if (
      !existing ||
      (compactTeamName(row.name) === key && compactTeamName(existing.name) !== key)
    ) {
      map.set(key, row);
    }
  }
...
export function canonicalFixtureKey(
  fixture: Pick<FixtureRow, "league" | "homeTeamId" | "awayTeamId" | "matchDate">,
  names: ReadonlyMap<number, string>,
) {
  const kickoff = Date.parse(fixture.matchDate);
  const kickoffBucket = Number.isFinite(kickoff)
    ? Math.round(kickoff / (60 * 60 * 1000))
    : fixture.matchDate;
  return [
    fixture.league,
    normaliseName(names.get(fixture.homeTeamId) ?? ""),
    normaliseName(names.get(fixture.awayTeamId) ?? ""),
    kickoffBucket,
  ].join("|");
}

export function preferredFixtureId<T extends { id: number }>(
  candidates: ReadonlyArray<T>,
  quotes: ReadonlyMap<number, Pick<QuoteRow, "capturedAt">>,
  now = Date.now(),
) {
  const ranked = [...candidates].sort((left, right) => {
    const leftCaptured = Date.parse(quotes.get(left.id)?.capturedAt ?? "");
    const rightCaptured = Date.parse(quotes.get(right.id)?.capturedAt ?? "");
    const leftFresh = Number.isFinite(leftCaptured) && now - leftCaptured <= MAX_QUOTE_AGE_SECONDS * 1000 ? 1 : 0;
    const rightFresh = Number.isFinite(rightCaptured) && now - rightCaptured <= MAX_QUOTE_AGE_SECONDS * 1000 ? 1 : 0;
...
async function buildMarketRows(db: QvmDb) {
  ...
  const names = new Map(teamsRows.map((team) => [team.id, team.name]));
  const now = Date.now();
  // Providers can use different team labels for the same fixture. Keep all
  // records in D1 for auditability, but display only the best quote for each
  // canonical fixture identity so an old provider alias cannot keep a stale
  // market visible beside its fresh replacement.
  const fixtureGroups = new Map<string, FixtureRow[]>();
  for (const fixture of fixtureRows.filter((item) => item.status === "SCHEDULED" && Date.parse(item.matchDate) >= now)) {
    const prediction = predictionsRows.find((row) => row.fixtureId === fixture.id);
    const quote = quoteMap.get(fixture.id);
    if (!prediction || !quote || quote.homeOdds <= 1 || quote.drawOdds <= 1 || quote.awayOdds <= 1) continue;
    const key = canonicalFixtureKey(fixture, names);
    const group = fixtureGroups.get(key) ?? [];
    group.push(fixture);
    fixtureGroups.set(key, group);
  }
  const displayFixtureIds = new Set<number>();
  for (const group of fixtureGroups.values()) {
    const preferred = preferredFixtureId(group, quoteMap, now);
    if (preferred !== null) displayFixtureIds.add(preferred);
  }
  ...
  return fixtureRows
    ...
    .filter((fixture) => displayFixtureIds.has(fixture.id))
    .map((fixture) => {
      const prediction = predictionsRows.find((row) => row.fixtureId === fixture.id);
      const quote = quoteMap.get(fixture.id);
      if (
...
export async function getOverview(db: QvmDb) {
  ...
  const latestAgents = (await db
    ...
    .limit(12)).filter(isHostedAgentRun).slice(0, 2);
  const settled = bets.filter((bet) => bet.status === "SETTLED");
...
function isHostedAgentRun(run: { payload: string | null }) {
  const payload = parseJsonPayload(run.payload);
  return Boolean(
    payload &&
      typeof payload === "object" &&
      (payload as Record<string, unknown>).source === "hosted-openai",
  );
}
...
export async function getAgentDashboard(db: QvmDb) {
  await ensureSeeded(db);
  const runtime = await getRuntimeEnv();
  const [rawRuns, backtests, risk] = await Promise.all([
    db
      .select()
      .from(agentRuns)
      .orderBy(desc(agentRuns.startedAt), desc(agentRuns.id))
  ...
  const runs = rawRuns.filter(isHostedAgentRun);
...
export async function getResearchDashboard(db: QvmDb) {
  ...
  const [snapshots, replays, experiments, scenarios, playerRows, heartbeats, calibration, sourceHealthRows, featureRows, availabilityRows] = await Promise.all([
    ...
    db.select().from(workerHeartbeats).where(eq(workerHeartbeats.workerName, "qvm-online")).limit(5),
    ...
    db
      ...
      .where(sql`${dataSourceHealth.source} <> 'API-Football' AND ${dataSourceHealth.source} <> 'Windows QVM worker'`)
      .orderBy(dataSourceHealth.source),
  ...
  return {
    capabilities: [
      ...
      "Champion/challenger experiments", "Decision replay and Auditor veto", "Hosted online cycle heartbeat",
      "Point-in-time feature store and source freshness gates", "De-vigged consensus and closing-line value",
      "OpenAI-hosted analyst, manager and auditor review",
    ],
    ...
    experiments,
    scenarios,
    heartbeats: heartbeats.filter((heartbeat) => heartbeat.workerName === "qvm-online"),
    calibration,
...
export async function syncWorkerSnapshot(
  ...
  const teamByName = buildTeamNameMap(teamRows);
  let teamsCreated = 0;
...
export async function refreshSportmonks(db: QvmDb) {
  ...
  try {
    ...
    const response = await fetch(`https://api.sportmonks.com${endpoint}?${params.toString()}`, { signal: AbortSignal.timeout(qvmTimeoutMs("provider")) });
    ...
    for (const resultDate of resultDates) {
      ...
      const dateResponse = await fetch(`https://api.sportmonks.com${dateEndpoint}?${params.toString()}`, { signal: AbortSignal.timeout(qvmTimeoutMs("provider")) });
    ...
    const teamsRows = await getTeamRows(db);
    const teamMap = buildTeamNameMap(teamsRows);
...
export async function refreshFootballDataOrg(db: QvmDb) {
  ...
  try {
    ...
    const response = await fetch(`https://api.football-data.org${endpoint}`, {
      ...
      signal: AbortSignal.timeout(qvmTimeoutMs("provider")),
    });
    ...
    const teamRows = await getTeamRows(db);
    const teamMap = buildTeamNameMap(teamRows);
...
export async function refreshResultsFromTheOddsAPI(db: QvmDb) {
  ...
  for (const league of LEAGUE_CONFIGS) {
    try {
      ...
      const response = await fetch(`https://api.the-odds-api.com/v4/sports/${league.oddsSport}/scores?${params.toString()}`, { signal: AbortSignal.timeout(qvmTimeoutMs("provider")) });
...
export async function refreshResultsFromTheSportsDB(db: QvmDb) {
  ...
  for (const fixture of unresolved.slice(0, 20)) {
    ...
    try {
      const response = await fetch(`https://www.thesportsdb.com/api/v1/json/3/searchevents.php?e=${query}`, { signal: AbortSignal.timeout(qvmTimeoutMs("provider")) });
...
export async function refreshFootballEnrichment(db: QvmDb) {
  ...
  try {
    ...
    const fixtureRows = await getFixtureRows(db);
    const teamsRows = await getTeamRows(db);
    const nameMap = buildTeamNameMap(teamsRows);
